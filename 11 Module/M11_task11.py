...
в класі повинен бути реалізований метод __iter__, який повертає ітератор. Ітератор – це будь-який об'єкт, 
...
Створіть у конструкторі набір випадкових векторів self.vectors завдовжки max_vectors за допомогою randrange. Атрибут current_index вказівник-індекс на поточний вектор зі списку vectors, необхідний для ітерування.
...
class Iterable:
    def __init__(self, max_vectors, max_points):
        ...
        self.vectors = [self._generate_vector() for _ in range(max_vectors)]

    def _generate_vector(self):
        x = randrange(self.max_points + 1)
        y = randrange(self.max_points + 1)
        return Vector(Point(x, y))
